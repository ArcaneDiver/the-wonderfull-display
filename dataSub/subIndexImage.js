...
(async () => {
    while (1) {
        ...
        var convert = child.spawnSync('sudo', ['convert', path.resolve(__dirname, '../img/input*.jpg'), '+append', '-crop', '100000x32+0+0', path.resolve(__dirname, '../img/converted/input.jpg')]); //+append serve per concatenare le immagini
        ...
        if (!fs.existsSync(path.resolve(__dirname, "../img/converted/input.jpg")) || numberOfFile <= 0) {
        
            fs.writeFileSync(path.resolve(__dirname, "../img/converted/input.jpg"), "");
        ...
        */
        ...
        console.log(bundledImage.bitmap);
        const { data: imageBuffer, width, height } = bundledImage.bitmap;

        
        matrix.setImageBuffer(imageBuffer, width, height);// setto l' immagine
